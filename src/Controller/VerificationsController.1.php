        ...
        try {
            ...
            if ($verificationId) {
                ...
                $verification = $this->Verifications->newEntity([
                    'email' => $value,
                    'phone' => '9999999999',
                    'email_otp_hash' => 'testhash',
                    'phone_otp_hash' => 'testhash',
                    'email_otp_expires' => date('Y-m-d H:i:s', strtotime('+10 minutes')),
                    'phone_otp_expires' => date('Y-m-d H:i:s', strtotime('+10 minutes')),
                    'email_verified' => false,
                    'phone_verified' => false,
                    'email_otp_sent_at' => date('Y-m-d H:i:s'),
                    'phone_otp_sent_at' => date('Y-m-d H:i:s'),
                    'email_resend_count' => 0,
                    'phone_resend_count' => 0
                ]);
                if (!$this->Verifications->save($verification)) {
            ...
            // Check cooldown
            // if (!$this->Verifications->canResendOTP($verification->id, 'email', 60)) {
            //     return $this->response->withType('application/json')
            //         ->withStringBody(json_encode([
            //             'success' => false,
            ...
            // Check resend limit
            // if ($this->Verifications->hasReachedResendLimit($verification->id, 'email', 5)) {
            //     return $this->response->withType('application/json')
            //         ->withStringBody(json_encode([
            //             'success' => false,
            ...
            if ($sent) {
                ...
                $this->Verifications->storeOTP($verification->id, 'email', $otp);
                // Update resend count
